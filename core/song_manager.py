...
class ChartSelectionResult:
    ...
    def model_dump(self):
        result = self.song.model_dump()
        result["selected_chart"] = self.chart.model_dump()
        return result
...
class SongSelector:
    ...
    def is_utage_chart(self, chart: Chart) -> bool:
        """判断是否为宴会场谱面
        ...
        Returns:
            是否为宴会场谱面
        """
        if chart.type == SongType.UTAGE:
            return True
        if chart.utage_kanji and chart.utage_kanji.startswith('['):
            return True
        return False
    ...
    def chart_matches_criteria(self, chart: Chart, criteria: SelectionCriteria) -> bool:
        """检查谱面是否符合选择条件
        ...
        Returns:
            是否符合条件
        """
        if not criteria.utage_only and self.is_utage_chart(chart):
            return False

        if criteria.utage_only and not self.is_utage_chart(chart):
            return False

        if criteria.song_type and chart.type != criteria.song_type:
            return False

        if criteria.difficulty and chart.difficulty != criteria.difficulty:
            return False
    ...
    def filter_charts(self, criteria: SelectionCriteria) -> list[tuple[Song, Chart]]:
        """根据选择条件过滤谱面
        ...
        for song in self.song_manager.get_all_songs():
            if criteria.genre and song.genre != criteria.genre:
                continue

            if criteria.version:
                if song.version != criteria.version and not any(c.version == criteria.version for c in song.charts):
                    continue

            for chart in song.charts:
                if self.chart_matches_criteria(chart, criteria):
    ...
    def select_random(self, criteria: SelectionCriteria) -> SelectionResult:
        ...
        Returns:
            选择结果，包含选中的谱面和相关信息
        """
        filtered_charts = self.filter_charts(criteria)
        ...
        selected_indices = random.choices(
            range(len(filtered_charts)),
            k=min(criteria.count, len(filtered_charts))
        )

        selected_songs = []
        for idx in selected_indices:
            song, chart = filtered_charts[idx]
            song_copy = Song(**song.model_dump())
            song_copy.charts = [chart]
            selected_songs.append(song_copy)

        return SelectionResult(
            songs=selected_songs,
            criteria=criteria,
            total_available=len(filtered_charts)
        )
